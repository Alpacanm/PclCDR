...
class UniCDR(nn.Module):
    ...
    def forward_user(self, domain_id, user, context_item, context_score, global_item, global_score):
        ...
        if self.training and (self.warmup == 0):
            ...
            if self.aggregator in ["Transformer"]:
                pos, p_embeddings, p_logits, p_min_distances= self.dis_list[dis_model_domain_id](specific_user,share_user,flag=True)
                neg, s_embeddings, n_logits, n_min_distances= self.dis_list[dis_model_domain_id](self.my_index_select(specific_user, random_label), share_user, flag=False)
        ...
        if self.aggregator in ["Transformer"] and self.training:
            ...
            return specific_user + share_user , p_min_distances, n_min_distances
